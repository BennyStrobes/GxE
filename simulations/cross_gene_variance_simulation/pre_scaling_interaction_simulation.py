...
def simulate_pre_scaling_interaction_expression(gene_geno, e_variable_proportion, proportional_amplification_scaling_factor, n_proportional_amplication_causal_snps_per_gene, orig_ge_h2, n_constant_causal_snps_per_gene, constant_ge_h2, n_interaction_causal_snps_per_gene, interaction_ge_h2):
	...
	mean_effects = np.dot(scaled_geno, orig_causal_effect_sizes) # equivalent to mean_effects = np.dot(standardized_geno, orig_causal_effect_sizes)*np.sqrt(SS)
	...
	interaction_mean_effects = np.dot(gxe, interaction_causal_effect_sizes)*np.sqrt(SS)
...
def variance_partitioning(YY, GG, EE,SS, prop_A=True, scale_estimate_w_reml=True):
	...
	#EE_scaled_geno = EE_scaled_geno - np.mean(EE_scaled_geno,axis=0)
...
raw_output_file = simulation_results_dir + 'pre_scaling_interaction_simulation_propAmpScale_' + str(proportional_amplification_scaling_factor) + '_sampleSize_' + str(sample_size) + '_raw_simulation_results2.txt'
...
################################
organized_output_file = simulation_results_dir + 'pre_scaling_interaction_simulation_propAmpScale_' + str(proportional_amplification_scaling_factor) + '_sampleSize_' + str(sample_size) + '_organized_simulation_results2.txt'
