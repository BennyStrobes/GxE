...
def simulate_total_proportional_amplification(gene_geno, e_variable_proportion, proportional_amplification_scaling_factor, n_proportional_amplication_causal_snps_per_gene, orig_ge_h2):
	...
	'''
	# Orig (should be identical)
	# Center environment
	E_c = EE - np.mean(EE)
	# Fit sqrt(S) = a + b * E_c by least squares
	sqrtS = np.sqrt(SS)
	X = np.vstack([np.ones_like(E_c), E_c]).T  # [1, E_c]
	aa, bb = np.linalg.lstsq(X, sqrtS, rcond=None)[0]
	G_const = aa * only_genetic_effects
	G_int   = bb * E_c * only_genetic_effects
	'''
	sqrtS = np.sqrt(SS)
	aa = np.mean(sqrtS)
	...
	return YY, standardized_geno, EE,SS, np.var(G_const,ddof=1), np.var(G_int, ddof=1)
...
raw_output_file = simulation_results_dir + 'total_PA_simulation_propAmpScale_' + str(proportional_amplification_scaling_factor) + '_sampleSize_' + str(sample_size) + '_raw_simulation_results2.txt'
...
for sim_iter in range(n_sims):
	...
	# Simulate data
	YY, GG, EE,SS, constant_var, interaction_var = simulate_total_proportional_amplification(gene_geno, e_variable_proportion, proportional_amplification_scaling_factor, n_proportional_amplication_causal_snps_per_gene, orig_ge_h2)
	...
	t.write(str(sim_iter) + '\t' + 'PA+interaction-VCA' + '\t' + 'constant_genetic_variance' + '\t' + str(h2_est + pa_mean_est) + '\t' + str(constant_var) + '\n')
	t.write(str(sim_iter) + '\t' + 'PA+interaction-VCA' + '\t' + 'interaction_genetic_variance' + '\t' + str(gxe_est) + '\t' + str(0.0) + '\n')
	t.write(str(sim_iter) + '\t' + 'PA+interaction-VCA' + '\t' + 'PA_interaction_genetic_variance' + '\t' + str(pa_interaction_est) + '\t' + str(interaction_var) + '\n')
	t.write(str(sim_iter) + '\t' + 'PA+interaction-VCA' + '\t' + 'total_genetic_variance' + '\t' + str(h2_est + gxe_est + pa_mean_est + pa_interaction_est) + '\t' + str(constant_var + interaction_var) + '\n')
	...
	# Print results to output file
	t.write(str(sim_iter) + '\t' + 'interaction-VCA' + '\t' + 'constant_genetic_variance' + '\t' + str(no_pa_mod_h2_est) + '\t' + str(constant_var) + '\n')
	t.write(str(sim_iter) + '\t' + 'interaction-VCA' + '\t' + 'interaction_genetic_variance' + '\t' + str(no_pa_mod_gxe_est) + '\t' + str(0.0) + '\n')
	t.write(str(sim_iter) + '\t' + 'interaction-VCA' + '\t' + 'PA_interaction_genetic_variance' + '\t' + str(0.0) + '\t' + str(interaction_var) + '\n')
	t.write(str(sim_iter) + '\t' + 'interaction-VCA' + '\t' + 'total_genetic_variance' + '\t' + str(no_pa_mod_h2_est + no_pa_mod_gxe_est) + '\t' + str(constant_var + interaction_var) + '\n')
t.close()
...
################################
organized_output_file = simulation_results_dir + 'total_PA_simulation_propAmpScale_' + str(proportional_amplification_scaling_factor) + '_sampleSize_' + str(sample_size) + '_organized_simulation_results2.txt'
